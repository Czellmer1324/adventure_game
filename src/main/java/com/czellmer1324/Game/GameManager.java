...
public class GameManager {
    ...
    private void viewInventory() {
        ...
        ChoiceValidation valid = utils.validateOption(sc.nextLine(), 1, 3);
        while (!valid.valid()) {
            IO.print("Please enter a valid choice: ");
            valid = utils.validateOption(sc.nextLine(), 1, 3);
        }

        LinkedList<Item> items = player.getInvent(valid.choice() - 1);
        if (items.isEmpty()) {
    ...
    private void useItem(LinkedList<Item> items, ItemType type) {
        ...
        ChoiceValidation valid = utils.validateOption(sc.nextLine(), 1, items.size());
        while (!valid.valid()) {
            IO.print("Please enter a valid choice: ");
            valid = utils.validateOption(sc.nextLine(), 1, items.size());
        }

        if (type.equals(ItemType.ARMOR)) player.applyArmor(valid.choice() - 1);
        else if (type.equals(ItemType.WEAPON)) player.applyWeapon(valid.choice() - 1);
        else {
            PotionResult result = player.usePotion(valid.choice() - 1);
            if (result.used()) {
